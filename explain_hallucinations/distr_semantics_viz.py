        ...
        for layer_idx in layer_indices:
            ...
            for i, c1 in enumerate(selected_concepts):
                for c2 in selected_concepts[i + 1 :]:
                    corr = np.corrcoef(activation_vectors[c1], activation_vectors[c2])[
                        0, 1
                    ]
                    if abs(corr) >= correlation_threshold:
            ...
            nx.draw_networkx_labels(
                G, pos, ax=ax, font_size=10, font_weight="bold", font_color="black"
            )

            for u, v, data in G.edges(data=True):
                corr = data["correlation"]
                color_val = (corr + 1) / 2  # map –1…1 ➔ 0…1 for colormap
                nx.draw_networkx_edges(
                    G,
                    pos,
                    edgelist=[(u, v)],
            ...
            sm = plt.cm.ScalarMappable(
                cmap=edge_cmap, norm=plt.Normalize(vmin=-1, vmax=1)
            )
            ...
            # --‑‑ Finishing touches -------------------------------------------
            ax.set_title(
                f"Layer {layer_idx}: Concept Relationship Network",
                fontsize=12,
                color="black",
            )
            ax.axis("off")
